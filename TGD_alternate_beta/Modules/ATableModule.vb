...
'Substance quantity at multiple facilities
Dim SQ As Long
Dim Import As Long
Dim Export As Long
Dim Conc As Long

'Tonnage at multiple facilities
Dim Tprod As Long
Dim T As Long

'Tonnage at 1 facility
Dim TTprod As Long
Dim TTform As Long
Dim TTind As Long
...
'Substance quantity at multiple facilities for each life cycle stage
SQ = Worksheets("PEC Calculator").Range("D12").Value

'Tonnage at multiple facilities
Tprod = CLng(SQ) / (CLng(Conc) / 100)
T = (CLng(SQ) / (CLng(Conc) / 100)) + Val(Import) - Val(Export)

'Tonnage at 1 facility = T (multiple facilities) * f main source
'This is what's used to determine Emission Factors for certain A tables / life cycles
If WorksheetFunction.IsNumber(Worksheets("PEC Calculator").Range("D32").Value) Then
TTprod = Tprod * Worksheets("PEC Calculator").Range("D32").Value 'This is fmain source for Production
End If

If WorksheetFunction.IsNumber(Worksheets("PEC Calculator").Range("E32").Value) Then
TTform = T * Worksheets("PEC Calculator").Range("E32").Value 'f main source for Formulation
End If

If WorksheetFunction.IsNumber(Worksheets("PEC Calculator").Range("F32").Value) Then
TTind = T * Worksheets("PEC Calculator").Range("F32").Value ' fmain source for Industrial Use
End If
